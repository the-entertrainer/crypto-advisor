...
export function calcTrend(prices) {
    ...
    const recentChg = window.length > 1
        ? ((window[window.length - 1] - window[0]) / window[0]) * 100
        : 0;
    if (spread > 2 && recentChg > 1)
        return 'strong_up';
    if (spread > 0.3)
        return 'up';
    if (spread < -2 && recentChg < -1)
...
export function calcEMASignal(prices) {
    if (prices.length < 26)
        return 'neutral';
    const e9 = ema(prices, 9);
    const e21 = ema(prices, 21);
...
export function calcMomentum(prices, period = 14) {
...
export function computeSignals(sparkline, volume24h, marketCap) {
