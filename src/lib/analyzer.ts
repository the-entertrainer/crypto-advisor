...
import type { CoinData, CoinInsight, TechnicalSignals, Signal, UserPrefs, GlobalData, FearGreedData } from './types';
...
function score(
  ...
  change24h: number,
  risk: UserPrefs['riskTolerance']
): { signal: Signal; confidence: number } {
  let s = 0;

  // RSI
  if (sig.rsi < 28) s += 4;
  else if (sig.rsi < 38) s += 2;
  else if (sig.rsi < 48) s += 1;
  else if (sig.rsi > 72) s -= 3;
  else if (sig.rsi > 62) s -= 1;

  // Trend
  const trendScore: Record<TechnicalSignals['trend'], number> = {
    strong_up: 3, up: 1, sideways: 0, down: -1, strong_down: -3,
  };
  s += trendScore[sig.trend];

  // EMA
  const emaScore: Record<TechnicalSignals['emaSignal'], number> = {
    golden_cross: 3, bullish: 1, neutral: 0, bearish: -1, death_cross: -3,
  };
  s += emaScore[sig.emaSignal];

  // Momentum
  if (sig.momentum > 8) s += 1;
  else if (sig.momentum < -15) s -= 2;

  // Volume confirming price
  if (sig.relativeVolume > 15 && change24h > 2) s += 2;
  else if (sig.relativeVolume > 10 && change24h > 0) s += 1;
  else if (sig.relativeVolume > 15 && change24h < -5) s -= 2;
...
function rsiText(rsi: number): string {
  if (rsi < 25) return 'virtually everyone who wanted to sell has already sold — this is as oversold as it gets';
  if (rsi < 35) return 'sellers are running out of energy and the downside pressure is fading';
  if (rsi < 46) return "it's been quietly pulling back without much drama — a decent entry zone";
  if (rsi < 56) return "sitting right in the middle with no strong conviction either way";
  if (rsi < 66) return "climbing with real momentum and still has room to run";
  if (rsi < 74) return "running strong but starting to get stretched — handle with care";
  return "overbought and overextended — history says a cooldown is coming";
}

function trendText(trend: TechnicalSignals['trend'], name: string): string {
  switch (trend) {
...
function buildNarrative(
  ...
  if (sig.emaSignal === 'golden_cross') {
    parts.push(
      "Its short-term moving average just crossed above the longer-term one — historically, this is exactly when things start accelerating upward."
    );
  } else if (sig.emaSignal === 'death_cross') {
    parts.push(
      "Its short-term average just crossed below the longer-term one — this usually signals more downside before a recovery."
    );
  }
  ...
  if (distToSup >= 0 && distToSup < 4) {
    parts.push(
      `It's sitting right above support at ${formatPrice(sig.support)} — a level buyers have defended multiple times before.`
    );
  }
  if (distToRes > 0 && distToRes < 7) {
    parts.push(
      `There's resistance just ahead at ${formatPrice(sig.resistance)} — breaking through there cleanly would be a strong signal.`
    );
  }

  if (sig.relativeVolume > 15) {
    parts.push(
      `Volume is noticeably elevated today — ${sig.relativeVolume.toFixed(0)}% of its market cap is trading hands, which usually means something real is happening.`
    );
  ...
  switch (signal) {
    ...
    case 'wait':
      parts.push(
        "No clear edge yet. Better to wait for a dip toward support or a confirmed break higher."
      );
      break;
    case 'avoid':
      parts.push(
        "Too many things pointing the wrong way. Better opportunities will come — sit this one out."
      );
      break;
  }
...
function buildBullets(coin: CoinData, sig: TechnicalSignals): string[] {
  const rsiLine =
    sig.rsi < 35
  ...
  const volLine = `Volume is ${
    sig.relativeVolume > 12 ? 'elevated' : sig.relativeVolume < 3 ? 'thin' : 'normal'
  } at ${sig.relativeVolume.toFixed(1)}% of market cap traded today`;
...
export function generateInsight(
  coin: CoinData,
  riskTolerance: UserPrefs['riskTolerance'] = 'balanced'
): CoinInsight {
...
export function marketNarrative(
  ...
  const btcText = global
    ? ` Bitcoin dominates ${global.btcDominance.toFixed(0)}% of the market${
