import { TradingStrategy, CoinMetrics, TradeSignal, ChainOfThought, AnalysisResult } from './types';
...
function generateCoinMetrics(coin: typeof TOP_CRYPTOS[0], strategy: TradingStrategy): CoinMetrics {
  ...
  const overallScore =
    (rsiScore * strategy.factors.rsiWeight +
  ...
  const indicatorVariance =
    Math.abs(rsiScore - 50) +
...
function createTradeSignal(coin: CoinMetrics, strategy: TradingStrategy): TradeSignal {
  const action =
    coin.confidence >= strategy.minConfidence && coin.buySignalStrength > 30
      ? 'buy'
  ...
  const winProbability = Math.min(
    95,
    coin.confidence * 0.8 + (coin.overallScore / 100) * 20
  );
...
function generateChainOfThought(coin: CoinMetrics, strategy: TradingStrategy): ChainOfThought {
  const steps = [
    {
      step: 1,
      title: 'RSI Analysis',
...
export async function analyzeWithMathematicalLogic(
  ...
  const reasoningChains = topOpportunities.map((signal) =>
    generateChainOfThought(signal.coin, strategy)
  );
  ...
  const sentiment =
    bullishCount / allMetrics.length > 0.6
      ? 'very_bullish'
  ...
  const volatilityLevel =
    avgVolatility > 75 ? 'extreme' : avgVolatility > 50 ? 'high' : avgVolatility > 25 ? 'medium' : 'low';
...
export const DEFAULT_STRATEGY: TradingStrategy = {
